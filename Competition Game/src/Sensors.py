...
class Sensor_Converter():
	...
	def convertSensor(self,sensor_temp): #returns converted sensor
		...
		if sensor_temp.type == 'Distance':
			created_sensor = Distance_Sensor(self.robot,(xNew,yNew),(wNew,hNew),sensor_temp.color,sensor_temp.direction)
		elif sensor_temp.type == 'Electromagnetic':
			created_sensor = Electromagnetic_Sensor(self.robot,(xNew,yNew),(wNew,hNew),sensor_temp.color,sensor_temp.direction)
		elif sensor_temp.type == 'Capacitive':
			created_sensor = Capacitive_Sensor(self.robot,(xNew,yNew),(wNew,hNew),sensor_temp.color,sensor_temp.direction)	
...
class Sensor():
	...
	def draw(self):
		...
		if rel_direction == 0:
			...
			self.object.topleft = (self.coords[0]+self.robot.object.topleft[0],self.coords[1]+self.robot.object.topleft[1])
		elif rel_direction == 1:
			self.object.size = (self.size[1],self.size[0])
			self.object.bottomleft = (self.robot.object.bottomleft[0]+self.coords[1],self.robot.object.bottomleft[1]-self.coords[0])
		elif rel_direction == 2:
			self.object.size = (self.size[0],self.size[1])
			self.object.bottomright = (self.robot.object.bottomright[0]-self.coords[0],self.robot.object.bottomright[1]-self.coords[1])
		elif rel_direction == 3:
			self.object.size = (self.size[1],self.size[0])
			self.object.topright = (self.robot.object.topright[0]-self.coords[1],self.robot.object.topright[1]+self.coords[0])
