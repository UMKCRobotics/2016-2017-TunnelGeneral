...
class Stage_Build(Stage):
	...
	def performAllStageActions(self):
		...
		if self.focused != None:
			pygame.draw.rect(self.screen,self.color_HIGHLIGHT,(self.focused.object.x,self.focused.object.y,self.focused.object.w,self.focused.object.h),3)
		return returnVal
	...
	def save_robot(self):
		...
		with open(os.path.join(robotpath,filename),'wb') as robotfile:
			...
			for sensor in sensor_list:
				robotfile.write(sensor.type+':'+str(sensor.OFFSETS)+':'+str((sensor.TOTAL_WIDTH,sensor.TOTAL_HEIGHT))+':'+str(sensor.color)+':'+str(sensor.direction)+'\n')
...
class Sensor_Creator():
	...
	def handleMouseEvent(self,event):
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			self.stage.focused = self.class_to_create(self.screen,self.stage,self.OFFSETS,(self.TOTAL_WIDTH,self.TOTAL_HEIGHT),self.color)
