...
int runMotorsTill(int value1, int value2, int pwm1, int pwm2) {
  ...
  while (on1 || on2) {
    ...
    if (on1) {
      ...
      else if (abs(duration1) <= startDiff) {
        int actualPWM1 = map(abs(duration1),0,startDiff,slowPWM,abs(pwm1));
    ...
    if (on2) {
      ...
      else if (abs(duration2) <= startDiff) {
        int actualPWM2 = map(abs(duration2),0,startDiff,slowPWM,abs(pwm2));
        analogWrite(MOT2_PWM,actualPWM2+5);
      }
      else if (abs(duration2) >= value2-slowDiff) {
        int actualPWM2 = map(abs(duration2),value2-slowDiff,value2,slowPWM,slowestPWM);
        analogWrite(MOT2_PWM,actualPWM2+5);
...
String goForward() {
  ...
  int forwCount = 2512;
  int actualDur = runMotorsTill(forwCount-25,forwCount+25,100,100+5);
  return "1";
}
...
String turnLeft() {
  ...
  int actualDur = runMotorsTill(1210,1210,100,-105);
  return "1";
}

String turnRight() {
  //int actualDur = runMotorsTill(1100,1100,"1r9\r","2f9\r");
  int actualDur = runMotorsTill(1215,1215,-100,105);
  return "1";
}
//END OF MOTOR STUFF
